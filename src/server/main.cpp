...
int main(int argc, char *argv[]) {
    ...
    unsigned int bloomSize = myStoi(argv[6]);
    string inputDir(argv[8]);
    if (inputDir.back() != '/') inputDir.append("/");

    // Prefix for the server and client named pipe files
    string serverFifo(FIFO_PATH), clientFifo(FIFO_PATH);
    serverFifo.append(SERVER_FIFO);
    clientFifo.append(CLIENT_FIFO);
    ...
    initFifo(FIFO_PATH, PERMS);
    for (unsigned int mon = 1; mon <= numMonitors; mon++) {
        fifoCreate(clientFifo + toString(mon), PERMS);
        fifoCreate(serverFifo + toString(mon), PERMS);
    }
